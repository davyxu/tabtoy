...
import(
	{{if .HasAnyRow}}"{{.ImportPackage}}"{{end}}
	{{if .HasAnyIndex}}"fmt"{{end}}
)

{{range $a, $row := .Rows}} {{range .Indexes}}
var {{$row.Name}}By{{.Name}} = make(map[{{.KeyType}}]*{{$.Package}}.{{$row.TypeName}})
{{end}} {{end}}
func init() {
	
	{{range $a, $row := .Rows}}
	
	RegisterIndexEntry("{{$row.Name}}", func( content interface{} ){
		
		config := content.(*{{$.Package}}.{{$.Name}})
		
		// {{$row.Name}}
		for _, def := range config.{{$row.Name}} {
			{{range .Indexes}}
...
func (self *goRowModel) TypeName() string {
...
func (self *goFileModel) HasAnyRow() bool {
